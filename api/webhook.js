...
module.exports = async function handler(req, res) {
  ...
  if (event.type === 'customer.subscription.deleted') {
    // TODO: Add STRIPE_SECRET_KEY to Vercel environment variables to enable auto-downgrade on cancellation.
    // Steps: Stripe dashboard → Developers → API Keys → create new restricted key → add to Vercel as STRIPE_SECRET_KEY
    ...
    // if (cust.email) await updateUserPlan(cust.email, 'free');
    const sub = event.data.object;
    console.log('Subscription cancelled for customer:', sub.customer, '— manual downgrade needed in Supabase until STRIPE_SECRET_KEY is added to Vercel');
  }
